...
const UserView = () => {
  ...
  async function fetchUser() {
    try {
      const res = await fetch(`http://localhost:3000/api/admin/viewmore/${userId}`);
  ...
  async function handleUpdate(event) {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:3000/api/admin/update/${userId}`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          ...
          email: user.email,
          profilePicture, // add other fields if necessary
        }),
      });
